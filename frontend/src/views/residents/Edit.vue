...
const fetchDataResident = async () => {
  await api.get(`/api/residents/${route.params.id}`).then(response => {
    const residentData = response.data.data;
    name.value = residentData.name;
    idCardNumber.value = residentData.id_card_number;
    phoneNumber.value = residentData.phone_number;
    maritalStatus.value = residentData.marital_status;
    active.value = residentData.active;
  });
}
...
const handleUpdate = async () => {
  ...
  await api.post(`/api/residents/${route.params.id}`, formData)
    .then(response => {
